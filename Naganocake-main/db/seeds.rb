...
# Items
Item.find_or_create_by!(name: "イチゴケーキ", genre_id: Genre.find_by(name: 'ケーキ').id) do |item|
  item.introduction = "イチゴ"
  item.price = 100
  item.is_deleted = false
end

Item.find_or_create_by!(name: "チョコケーキ", genre_id: Genre.find_by(name: 'ケーキ').id) do |item|
  item.introduction = "チョコ"
  item.price = 120
  item.is_deleted = true
end

Item.find_or_create_by!(name: "抹茶ケーキ", genre_id: Genre.find_by(name: 'ケーキ').id) do |item|
...
10.times do |i|
  Item.find_or_create_by!(name: "商品#{i+1}", genre_id: Genre.find_by(name: 'ケーキ').id) do |item|
